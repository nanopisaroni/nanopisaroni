#!/usr/bin/env python3
"""Insert Naval section and fix numbering - directly in file."""
import re

with open("/home/nanobot/sitio/panteon.html", "r") as f:
    html = f.read()

# ===== 1. Insert Naval's section after Paul Graham's closing </section> =====
# Find the exact insertion point: after PG's </section> and before Yudkowsky
marker = '</section>\n\n<section class="thinker" id="thinker-10">\n<h2><span class="num">#10</span> Eliezer Yudkowsky'

naval_section = """</section>
...
<section class="thinker" id="thinker-11">
<h2><span class="num">#11</span> Eliezer Yudkowsky"""

if marker in html:
    html = html.replace(marker, naval_section)
    print("Naval section inserted successfully!")
else:
    print("ERROR: Marker not found!")
    # Find what's actually there
    idx = html.find('</section>\n\n<section class="thinker"')
    if idx >= 0:
        print(f"Found section transition at {idx}")
        print(f"Context: {repr(html[idx:idx+80])}")

# ===== 2. Fix remaining numbering (10>11, 11>12, ... 14>15, 15>16) =====
# Apply HIGH to LOW to avoid cascade
renumber = [
    ('15', '16'),
    ('14', '15'),
    ('13', '14'),
...
for i, line in enumerate(lines):
    if line.startswith('<a href="#thinker-1">#1 Jorge Luis Borges</a>'):
        lines[i] = new_toc
...
print(f"\nSection IDs ({len(set(sections))}):")
for s in sorted(set(sections)):
    print(f"  {s}")

# Check Conexiones gone
if '<h2>Conexiones' in output:
    print("WARNING: Conexiones still present!")
else:
    print("Conexiones: removed OK")

print(f"Naval mentions: {output.count('Naval Ravikant')}")
